  ...
  describe Rewriter::WithinScope do
    ...
    describe '#process' do
      ...
      it 'matches multiple block nodes' do
        block_nodes = []
        scope =
          Rewriter::WithinScope.new(instance, { type: 'block' }, { stop_when_match: false }) { block_nodes << node }
        scope.process
      ...
      it 'matches only one block node if no recursive' do
        block_nodes = []
        scope =
          Rewriter::WithinScope.new(instance, { type: 'block' }, { stop_when_match: true }) { block_nodes << node }
        scope.process
