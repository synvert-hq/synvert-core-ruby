...
module Synvert::Core
  describe Rewriter::Instance do
    ...
    it 'parses within_node' do
      scope = double
      block = Proc.new {}
      expect(Rewriter::WithinScope).to receive(:new).with(instance, type: 'send', message: 'create', &block).and_return(
        scope
      )
...
" \
            ''
        expect(Dir).to receive(:glob).with('./spec/**/*_spec.rb').and_return(%w[spec/models/post_spec.rb])
      ...
      it 'does not write if file content is not changed' do
        instance =
          Rewriter::Instance.new rewriter, 'spec/spec_helper.rb' do
            with_node type: 'block', caller: { receiver: 'RSpec', message: 'configure' } do
              unless_exist_node type: 'send', message: 'include', arguments: %w[FactoryGirl::Syntax::Methods] do
        ...
        ' \
            ''
        expect(Dir).to receive(:glob).with('./spec/spec_helper.rb').and_return(%w[spec/spec_helper.rb])
      ...
      it 'does not read file if already read' do
        ...
        ' \
            ''
        expect(Dir).to receive(:glob).with('./spec/spec_helper.rb').and_return(%w[spec/spec_helper.rb]).twice
...
" \
            ''
        expect(Dir).to receive(:glob).with('./spec/**/*_spec.rb').and_return(%w[spec/models/post_spec.rb]).twice
        expect(File).to receive(:read).with('spec/models/post_spec.rb').and_return(input)
        expect(File).to receive(:write).with('spec/models/post_spec.rb', output)
